 ...
 *
 * Notes:
 *        - The algorithm looks like a 16 round variation of TEA.
 *          The magic constants also seem to be nonstandard.
 ...
 *          to turn it off to build mi4code just for decoding or
 *          for handling only 010201 images.
 *
 * Thanks to
 *
 *        Daniel Stenberg      for hosting the project.
 *        Benjamin Larsson     for the MDI P640 key.
 *
 * (c) MrH 2006
 *
 */
...
mi4hdr_dsa_check()
{
  if (mi4hdr.version != 0x10301) {
    fprintf(stderr,
	    "The mi4 has version %x header. Only version 10301 uses DSA!\n"
	    "Nothing to do!\n", mi4hdr.version);
    return 0;
  }
...
key_scan(FILE * ifp, FILE * kfp, int complete)
{
  ...
  printf("Scanning potential keys\n");
  ...
  if (!count) {
    printf("No potential keys found\n");
  }
	 ...
	 const char * str)
{
  ...
  while (offs >= 0) {
	...
	printf("Duplicate match for %s at %08lx and %08lx\n",
	       str, *o_pos, pos + offs);
	return -1;
      }
...
bl_patch(FILE * ifp, FILE * ofp, struct dsa_key * dkey)
{
  ...
  printf("Scanning the bootloader for existing DSA keys\n");
  ...
  for (i = 0, key = &dsa_keytable[0]; i < NUM_DSA_KEYS; i++, key++) {
    ...
    if (p_pos && q_pos && g_pos && y_pos) {
      printv(V_LOW, "found!\n");
      printf("Found DSA key '%s', ", key->name);
      printf("p at %lx, q at %lx, g at %lx, y at %lx\n",
      ...
      printf("Bootloader patched with key '%s'!\n", dkey->name);
...
myopt(int argc, char * argv[], const char * optstr, const char ** optarg)
{
  ...
  if (c[1] == ':') {
    ...
    if (c[2] == ':') {
      ...
      *optarg = opt[1] ? opt + 1 : argv[optind++];
      opt = NULL;
    }
  } else {
    opt++;
...
cmd_decrypt(int argc, char * argv[])
{
  ...
  while ((c = myopt(argc, argv, "lsvh", NULL)) > 0) {
    switch (c) {
    case 'l':
      print_tea_keys();
      return 0;
  ...
  if (argc > 0) {
    ...
    printf("Decrypting with key %08x %08x %08x %08x\n",
	   key[0], key[1], key[2], key[3]);
    ...
    if (decode_ok(ofp, !strip_header)) {
      ...
      printf("Verification failed (maybe wrong key?)!\n");
      return 1;
    }
  }
  ...
  for (i = 0; i < NUM_TEA_KEYS; i++) {
    ...
    if (decode_ok(ofp, !strip_header)) {
      printv(V_LOW, "ok!\n");
      printf("Decrypted ok with key '%s' (%08x %08x %08x %08x)\n",
	     kp->name, kp->key[0], kp->key[1], kp->key[2], kp->key[3]);
      return 0;
    } else {
      printv(V_LOW, "fail\n");
  ...
  printf("Unable to decrypt the file (maybe the key is not known)!\n");
...
cmd_encrypt(int argc, char * argv[])
{
  ...
  while ((c = myopt(argc, argv, "lnvh", NULL)) > 0) {
    switch (c) {
    case 'l':
      print_tea_keys();
      return 0;
  ...
  printf("Encrypting with key %08x %08x %08x %08x\n",
	 key[0], key[1], key[2], key[3]);
  ...
  if (crc != mi4hdr.crc32) {
    if (fix_crc) {
      printf("CRC32 corrected (%08x -> %08x)\n", mi4hdr.crc32, crc);
      mi4hdr.crc32 = crc;
      mi4hdr_write(ofp);
    } else {
      printf("CRC32 mismatch detected (%08x <-> %08x)\n", mi4hdr.crc32, crc);
    }
  }
...
{
  printf("\n"
	 "Usage:\tmi4code build [options] <infile> <outfile>\n"
	 "\n"
	 "options:\n"
...
cmd_build(int argc, char * argv[])
{
  ...
  while ((c = myopt(argc, argv, "23vh", NULL)) > 0) {
    switch (c) {
    case '2':
      ver = 0x10201;
      return 0;
  ...
  if (ftell(ofp) < (0xec + sizeof(struct mi4hdr))) {
    fprintf(stderr, "File too small, at least 0xec bytes needed!\n");
    return 1;
  }
...
cmd_sign(int argc, char * argv[])
{
  ...
  while ((c = myopt(argc, argv, "lvh", NULL)) > 0) {
    switch (c) {
    case 'l':
      print_dsa_keys();
      return 0;
...
cmd_verify(int argc, char * argv[])
{
  ...
  if (!memcmp(mi4hdr.dsa_r, dummy_r, sizeof(mi4hdr.dsa_r)) &&
    ...
    printf("The mi4 is signed with 'dummy' signature. It is not possible "
	   "to verify it.\n");
    return 0;
  }
  ...
  for (i = 0; i < NUM_DSA_KEYS; i++) {
    ...
    if (verify_image(ifp, dkey->p, dkey->q, dkey->g, dkey->y,
      ...
      printf("Image verified ok with DSA key '%s'\n", dkey->name);
      return 0;
    }
  }

  printf("No suitable DSA key found (or signature is incorrect)!\n");
...
cmd_keyscan(int argc, char * argv[])
{
  ...
  while ((c = myopt(argc, argv, "vh", NULL)) > 0) {
    switch (c) {
    case 'v':
      verbose++;
      break;
...
cmd_hexenc(int argc, char * argv[])
{
  ...
  if (sum != hexhdr.sum2) {
    if (fix_csum) {
      printf("Checksum corrected (%08x -> %08x)\n", hexhdr.sum2, sum);
      hexhdr.sum2 = sum;
      hexhdr_write(ofp);
    } else {
      printf("Checksum mismatch detected (%08x <-> %08x)\n", hexhdr.sum2, sum);
    }
  }
...
main(int argc, char * argv[])
{
  ...
  printf("mi4code " VERSION VERSION_EXT " (c) by MrH 2006\n");
