 ...
 * 4. When the wheel lights up, release the centre button
 *
...
#define VERSION "v0.2.4-alpha" // NAND access extension by Virtuoso015
...
#endif
/* Interface and endpoint numbers of the default
 * manufacture mode
 */
#define E200_INTERFACE 0
#define E200_ENDPOINT  1

#define E200_TIMEOUT    5000	// in milliseconds ??
...
void
put_le32(void * ptr, uint32_t val)		//uint32 to little endian
{
  uint8_t * b = ptr;
...
uint32_t
get_le32(void * ptr)                  // little endian to uint32
{
  uint8_t * b = ptr;
  uint32_t val;
...
send_dev(usb_dev_handle * ud, const void * data, uint32_t len)
{
  ...
  while (len > 0) {
    todo = len > BUFSIZE ? BUFSIZE : len;	// not more than 64 bytes at a time
...
int cmd_nandread(int, char **);
int cmd_nandwrite(int, char **);
int cmd_nanddetect(int, char **);
...
struct cmd cmd_table[] = {
  ...
  { "nandread",   cmd_nandread,   "read 512-byte sectors from NAND drive" },
  { "nandwrite",  cmd_nandwrite,  "write 512-byte sectors to NAND drive" },
  { "nanddetect", cmd_nanddetect, "detect hidden partition starting sector and size"}
};
...
{
  static const uint32_t len = sizeof(e200_code);
  usb_dev_handle * ud;
  int ret;
  ...
  ret = send_dev(ud, e200_code, len);	// this sends the usb stub to the device
...
/* NAND ACCESS EXTENSION START */

#define NAND_BUFFER    I2C_ROM_BUFFER
#define NAND_TIMEOUT   120000 

int set_nand_addr(usb_dev_handle * ud,unsigned long addr)
{
	int ret;
	
	put_le32(buffer,addr);
	
	ret= usb_control_msg(ud,REQ_OUT,REQ_NANDADDR,0,0,
			buffer,sizeof(addr),E200_TIMEOUT);
	
	if (ret < 0) {
    	fprintf(stderr, "Set nand address - error (%d, %s)\n", ret, strerror(-ret));
    	exit(1);
  }
...
int nand_read_from_mem(usb_dev_handle* ud, FILE* fp, int total)
{
			int ret,len;
...
help_nandread()
{
  fprintf(stderr, "\nUsage:\te200tool nandread  <nand_start_sector> <number_of_sectors> <outfile>\n\n");
  exit(1);
}

int
cmd_nandread(int argc, char * argv[])
{
  usb_dev_handle * ud;
  unsigned long nand_bl_addr, total ;
  int ret,todo;
  char * fn;
  FILE * fp;
  ...
  if (!parse_uint32(argv[0],(uint32_t *) &nand_bl_addr) || !parse_uint32(argv[1],(uint32_t *) &total)) {
    exit(1);
  }
  ...
  fprintf(stderr, "Reading from block=%08u to %08u to '%s'\n",
	  nand_bl_addr, nand_bl_addr+total, fn);
  ...
  while (total) {
    todo= MIN(total,16); // only read 16 blocks at a time (or 8KB)
    
    set_ptr(ud, NAND_BUFFER);			// set memory pointer where data is to be stored from nand
    
    set_nand_addr(ud,nand_bl_addr);           // set the block address
    
    // should return number of blocks read
    ret = usb_control_msg(ud, REQ_OUT, REQ_NANDREAD, todo, 0,
						NULL, 0, NAND_TIMEOUT);
    ...
    nand_read_from_mem(ud,fp,todo*BLOCK_SIZE);
    
    fprintf(stderr, "\rReading from %08u", nand_bl_addr);

    nand_bl_addr  += todo;
    total -= todo;
  }

  fprintf(stderr, "\rReading from %08u\nRead done!\n", nand_bl_addr);
...
int nand_write_to_mem(usb_dev_handle* ud, FILE* fp, int total)
{
			int ret,todo;
			...
			while (total) {
				...
				if(todo != fread(buffer,1,todo,fp)) {
					fprintf(stderr,"\nFile read error.\t*DANGER* Please retry !!!\n");				
					exit(1);
				}

				//TODO: make sure spurious data is not written, i.e, when len < BUFSIZE-1 
				
				ret = usb_control_msg(ud, REQ_OUT, REQ_WRITE, 0, 0,
						buffer, todo, E200_TIMEOUT);

				if (ret < 0) {
				  fprintf(stderr, "\nControl message (%d, %s)\t*DANGER* Please retry !!!\n", ret, strerror(-ret));
				  exit(1);
				}
...
help_nandwrite()
{
  fprintf(stderr, "\nUsage:\te200tool nandwrite  <infile> <nand_start_sector> <number_of_sectors>\n"
									"\nPlease provide infile of a size that is multiple of 512 bytes.\n");
  exit(1);
}

int
cmd_nandwrite(int argc, char * argv[])
{
  usb_dev_handle * ud;
  unsigned long nand_bl_addr, total ;
  int ret,todo;
  ...
  if (!parse_uint32(argv[1],(uint32_t *) &nand_bl_addr) || !parse_uint32(argv[2],(uint32_t *) &total)) {
    exit(1);
  }
  ...
  fprintf(stderr, "Writing from block=%08u to %08u from '%s'\n",
	  nand_bl_addr, nand_bl_addr+total, fn);
  ...
  while (total) {
    ...
    todo= MIN(total,16); // only write 16 blocks at a time (or 8KB)
    
    set_ptr(ud, NAND_BUFFER);			// set memory pointer where data is to be stored temporarily
         
    nand_write_to_mem(ud,fp,todo*BLOCK_SIZE);		// write data from file to memory
    
    set_ptr(ud, NAND_BUFFER);			// reset memory pointer
     
    set_nand_addr(ud,nand_bl_addr);           // set the block address
    
        
    // will return success on writing the blocks to nand
    ret = usb_control_msg(ud, REQ_OUT, REQ_NANDWRITE, todo, 0,
						NULL, 0, NAND_TIMEOUT);
    ...
    fprintf(stderr, "\rWriting to %08u", nand_bl_addr);

    nand_bl_addr  += todo;
    total -= todo;
  }

  fprintf(stderr, "\rWriting to %08u\nWrite done!\n", nand_bl_addr);
...
int
cmd_nanddetect(int argc, char * argv[])
{
	 int len;char buffer[4];
	 uint32_t sector,length;
   FILE *fp;
   char *fn= "/tmp/mbr.bin";
   
   argc= 3;
   argv[0] = "0";	// start_sector		
   argv[1] = "1"; // no. of sectors
   argv[2] = fn;  // filename
   
   cmd_nandread(argc,argv);
   ...
   if (fseek(fp, 470, SEEK_SET) < 0) {  // 470 is address of hidden partition LBA
    perror("fseek");
    exit(1);
   }
   
   len= fread(buffer,1,4,fp);
   if(len != 4) {
   	fprintf(stderr,"File read error (%d, %s)\n",len,strerror(-len));
   	exit(0);
   }
   
   sector = get_le32(buffer);
   ...
   length = get_le32(buffer);
   
   printf("\nHidden partition : start_sector= %u  number_of_sectors= %u\n",sector,length);
